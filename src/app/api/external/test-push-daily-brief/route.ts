...
// Reuse the same markdown generation logic
...
function generateMarkdown(): { date: string; markdown: string } {
  ...
  if (inspirations.length === 0) {
    ...
    for (const v of inspirations.slice(0, 8)) {
      ...
      if (v.why_good) lines.push(`  - 为什么值得学习: ${v.why_good}`);
      lines.push(`  - ${link}`);
    }
    lines.push('');
  }
  ...
  return { date: today, markdown: lines.join('\n') };
}
...
export async function POST(req: NextRequest) {
  ...
  const { date, markdown } = generateMarkdown();

  const results: Record<string, unknown> = { date, markdown, markdown_sent: false };

  // Push to n8n webhook
  const n8nUrl = process.env.N8N_WEBHOOK_URL;
  if (n8nUrl && (target === 'n8n' || target === 'all')) {
    try {
      const resp = await fetch(n8nUrl, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ date, markdown, source: 'tiktok-trend-packaging-agent' }),
      });
      results.n8n_status = resp.ok ? 'sent' : `failed (${resp.status})`;
    } catch (e: unknown) {
      results.n8n_status = `error: ${e instanceof Error ? e.message : 'unknown'}`;
    }
  } else if (target === 'n8n') {
    results.n8n_status = 'skipped: N8N_WEBHOOK_URL not set';
  }

  // Push to OpenClaw webhook
  const openclawUrl = process.env.OPENCLAW_WEBHOOK_URL;
  if (openclawUrl && (target === 'openclaw' || target === 'all')) {
    try {
      ...
      results.openclaw_status = resp.ok ? 'sent' : `failed (${resp.status})`;
    } catch (e: unknown) {
      results.openclaw_status = `error: ${e instanceof Error ? e.message : 'unknown'}`;
    }
  } else if (target === 'openclaw') {
    results.openclaw_status = 'skipped: OPENCLAW_WEBHOOK_URL not set';
  }

  // If no webhook configured, just return the markdown
  if (!n8nUrl && !openclawUrl) {
    results.note = 'No webhook configured. Markdown returned only. Set N8N_WEBHOOK_URL or OPENCLAW_WEBHOOK_URL in .env.local to enable push.';
  }
